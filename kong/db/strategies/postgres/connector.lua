...
function _mt:insert_lock(key, ttl, owner)
  local ttl_escaped = concat {
                        "TO_TIMESTAMP(",
                        self:escape_literal(floor(now_updated() + ttl / 1000)),
                        ") AT TIME ZONE 'UTC'"
                      }
  ...
  if res[3] then
    if res[3].affected_rows == 0 then
      return nil, fmt("The key '%s' is already locked. Please try again later.", key, owner)
    elseif res[3].affected_rows == 1 then
      return true
    end
  end
